...
import 'package:frontend_fisio/features/realtime_pose/service/realtime_pose_service.dart';
import 'package:frontend_fisio/features/realtime_pose/service/realtime_socket_service.dart';
...
class ExerciseBloc extends Bloc<ExerciseEvent, ExerciseState> {
  ...
  static const Duration _incrementDebounce = Duration(milliseconds: 1000); 
  ...
  void _onUpdateSocketData(UpdateSocketData event, Emitter<ExerciseState> emit) {
      // 1. Determine Match
      final currentLatihan = state.latihanData?.latihan;
      final targetName = currentLatihan?.namaLatihan.toLowerCase() ?? ''; // Nested
      final predicted = event.predictedLabel.toLowerCase();
      
      // Check if predicted label matches the target exercise
      final isMatch = targetName.isNotEmpty && (targetName.contains(predicted) || predicted.contains(targetName));
      
      // 🆕 Backend Logic: isValid == true AND isMatch
      // Backend sets isValid based on confidence > 0.8
      final isCorrect = event.isValid && isMatch;
      
      // Logic for wrong: Valid but mismatch OR Invalid but confidence > 0.5 (trying but failing)
      // Simplify: if Socket says invalid -> wrong. If socket valid but mismatch -> wrong.
      ...
      final updatedHistory = List<ExerciseHistoryItem>.from(state.history)..add(historyItem);
      
      emit(state.copyWith(
          predictedLabel: event.predictedLabel,
          currentAccuracy: event.confidence,
      ...
      // 5. Auto Increment Logic
      if (state.isRunning && isCorrect) {
          final now = DateTime.now();
          if (_lastIncrementTime == null || now.difference(_lastIncrementTime!) > _incrementDebounce) {
              print('✅ [AUTO_INC] Correct Pose: $predicted ($targetName)');
